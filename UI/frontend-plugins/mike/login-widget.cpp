...
int curl_string_callback(void *data, int size, int cnt, void *user)
{
	...
	str->insert(str->end(), (const char *)data,
		    (const char *)data + (size * cnt));
	return cnt;
}
...
LoginWidget::LoginWidget(QWidget *parent, LoginCallback _cb)
	: QWidget(parent), cb(_cb)
{
	...
	QObject::connect(login_button, &QPushButton::clicked, [&]() {
		...
		std::string err;
		std::string res =
			Login(un.toUtf8().constData(), pw.toUtf8().constData());
