...
Q_DECL_IMPORT void
qt_blurImage(QPainter *p, QImage &blurImage, qreal radius, bool quality,
	     bool alphaOnly,
...
struct Animation {
	...
	Animation(Type _easing, int _duration)
		: easing{_easing}, duration{_duration}
	{
	}
...
inline QPixmap drawShadowEllipse(qreal radius, qreal elevation,
				 const QColor &color)
{
	...
	{ // draw ellipes
		...
		p.drawEllipse(QRectF(0, 0, px.size().width(),
				     px.size().height())
				      .center(),
